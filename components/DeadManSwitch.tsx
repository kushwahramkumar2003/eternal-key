...
const PROGRAM_ID = new PublicKey(
  "6UorpWUdDG7jqd9zfR4pfBVhn9PrDCa59VdnfJr6m25q"
);
...
const DeadManSwitch: FC = () => {
  ...
  const activateSwitch = async (seconds: number) => {
    ...
    try {
      ...
      if (balance < amountInLamports) {
        toast.error(
          `Insufficient funds. You need at least ${depositAmount} SOL. Current balance: ${(
      ...
      await program.methods
        .initialize(new BN(deadline), new PublicKey(beneficiaryAddress), seed)
        .accounts({
          owner: publicKey,
          escrow: escrowPDA,
  ...
  const calculateTotalSeconds = (duration: ExtendDuration): number => {
    ...
    const future = addYears(
      addMonths(addDays(now, duration.days), duration.months),
      duration.years
    );
        ...
        <div className="space-y-8">
          <div>
            <h2 className="text-2xl font-bold text-white mb-2">
              Time-Locked Succession
            </h2>
            <p className="text-gray-600 dark:text-gray-400">
              Set up automatic transfer of funds if you don&apos;t check in
              regularly
            </p>
          </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2 text-sm">
                          <p className="text-gray-300">
                            ...
                            {escrow.account.beneficiary.toString()}
                          </p>
                          <p className="text-gray-300">
                            <span className="text-gray-400">Balance:</span>{" "}
                            {formatSolBalance(escrow.balance)}
                          </p>
                          <p className="text-gray-300">
                            ...
                            {new Date(
                              escrow.account.lastCheckin.toNumber() * 1000
                            ).toLocaleString()}
                          </p>
                          <p className="text-gray-300">
                            <span className="text-gray-400">Deadline:</span>{" "}
                            {new Date(
                              escrow.account.deadline.toNumber() * 1000
                            ).toLocaleString()}
                          </p>
                      ...
                      <div className="space-y-4">
                        ...
                        publicKey?.toString() ? (
                          <Button
                            onClick={() => {
                              if (
                                escrow.account.deadline.toNumber() <=
                                Date.now() / 1000
                              ) {
                                claimEscrow(
                                  escrow.pubkey,
                                  escrow.account.beneficiary
                                );
                              } else {
                                toast.error(
                                  `Cannot claim yet. Time remaining: ${escrow.timeRemaining}.`
                                );
                              }
                            }}
                            variant="default"
                            ...
                            <>
                              ...
                              <div className="flex gap-3">
                                <Button
                                  ...
                                  disabled={
                                    !extendDuration.days &&
                                    !extendDuration.months &&
                                    !extendDuration.years
                                  }
                                >
                                  <span className="relative z-10 flex items-center justify-center gap-2">
                                    <span>Check In</span>
